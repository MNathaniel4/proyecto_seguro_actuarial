...
from utils.preprocessing import ActuarialFeatureEngineer  # noqa: F401
...
def variables_modelo(df):
    ...
    if "costo_esperado_anual_mxn" in df_modelo.columns:
        ...
        df_modelo["log_costo"] = np.log(costo)
    if {"costo_esperado_anual_mxn", "prima_mensual_mxn"}.issubset(df_modelo.columns):
        ...
        df_modelo["ratio_costo_prima"] = df_modelo["costo_esperado_anual_mxn"] / prima
...
def asegurar_columnas_entrenamiento(X):
    X = X.copy()
...
def resumen_preprocesador(pipeline):
...
def obtener_importancias(pipeline):
...
def mostrar_modelado(df, metadata):
    ...
    with col1:
        ...
        metrica_reg = st.selectbox("Métrica de regresión", metricas_reg, index=metricas_reg.index("MAE"))
    ...
    modelo_importancia = col_modelo.selectbox("Modelo", list(modelos_tipo.keys()))
